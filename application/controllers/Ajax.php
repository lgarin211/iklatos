...
class Ajax extends CI_Controller
{
    ...
    //             }
    //         } else if ($type == 'keluar') {
    //             $cekabsenkeluar =     $cekabsen = $this->db->query("SELECT * FROM tabel_detail_absen WHERE nis = '$nis' AND masuk = '1' AND tanggal_absen = '$tglskrg' ")->num_rows();
    //             if ($cekabsen == 0) {
    ...
    //                 $this->db->query("UPDATE tabel_detail_absen SET keluar = '1', jam_pulang = '$tanggalFormat' WHERE nis ='$nis' AND tanggal_absen = '$tanggal' AND masuk = '1'");
    //                 $datasiswa = $this->M_siswa->detailsiswa($nis);
    //                 $datasiswa2 = $datasiswa[0];
    ...
    if ($this->M_siswa->CekSiswa($nis) < 1) {
        ...
        if ($type == 'masuk') {
            if ($cekabsen > 0) {
                ...
                $pesannya = 'Ananda ' . $datasiswa2['nama_siswa'] . ' telah absen masuk hari ini pada pukul ' . date('H:i:s');
                $this->M_notif->send_whatsapp($datasiswa2['no_telepon'], $pesannya);
                echo json_encode(array_merge($datasiswa2, ['status' => 'true', 'gambar' => $datasiswa2['gambar']]));
            }
        } else if ($type == 'keluar') {
            $cekabsenkeluar = $this->db->query("SELECT * FROM tabel_detail_absen WHERE nis = '$nis' AND masuk = '1' AND tanggal_absen = '$tglskrg' ")->num_rows();
            if ($cekabsen == 0) {
                $result3 = ['status' => 'false', 'alert' => 'danger', 'message' => 'Kamu tidak bisa absen keluar , karna tidak ada data absen masuk kamu hari ini!'];
                echo json_encode($result3);
            } else if ($cekabsenkeluar > 0) {
                ...
                $pesannya = 'Ananda ' . $datasiswa2['nama_siswa'] . ' telah absen keluar hari ini pada pukul ' . date('H:i:s');
                $this->M_notif->send_whatsapp($datasiswa2['no_telepon'], $pesannya);
                $result3 = ['status' => 'false', 'alert' => 'success', 'message' => 'Kamu sudah absen keluar hari ini, selamat beristirahat!', 'gambar' => $datasiswa2['gambar']];
                echo json_encode($result3);
            }
        }
    }
